 ...
 * {@link org.phenotips.vocabulary.internal.solr.MondoDiseaseOntology}.
 */
public class MondoDiseaseOntologyTest
{
    @Rule
    public final MockitoComponentMockingRule<Vocabulary> mocker =
        new MockitoComponentMockingRule<Vocabulary>(MondoDiseaseOntology.class);
        ...
        throws ComponentLookupException, IOException, SolrServerException, CacheException
    {
        ...
        when(externalServicesAccess.getTermCache("mondo")).thenReturn(this.cache);
        this.server = mock(SolrClient.class);
        when(externalServicesAccess.getSolrConnection("mondo")).thenReturn(this.server);
        this.ontologyService = this.mocker.getComponentUnderTest();
        this.ontologyServiceResult =
            this.ontologyService.reindex(this.getClass().getResource("/mondo-test.obo").toString());
    }

    @Test
    public void testMondoDiseaseOntologyReindex()
        throws ComponentLookupException, IOException, SolrServerException, CacheException
    {
    ...
    @Test
    public void testtMondoDiseaseOntologyVersion() throws SolrServerException, IOException
    {
        ...
        when(versionDoc.getFieldValue("version")).thenReturn("releases/2019-04-29/reasoned.owl.owl");
        Assert.assertEquals("releases/2019-04-29/reasoned.owl.owl", this.ontologyService.getVersion());
    }

    @Test
    public void testtMondoDiseaseOntologyDefaultLocation()
    {
        String location = this.ontologyService.getDefaultSourceLocation();
        Assert.assertNotNull(location);
        Assert.assertTrue(location.endsWith("mondo.obo"));
        Assert.assertTrue(location.startsWith("http"));
    }

    @Test
    public void testMondoDiseaseOntologySuggestTermsBlank() throws ComponentLookupException
    {
        Assert.assertTrue(this.mocker.getComponentUnderTest().search("", 0, null, null).isEmpty());
    }

    @Test
    public void testMondoDiseaseOntologySuggestTermsIsId() throws ComponentLookupException, SolrServerException,
        IOException
    {
        QueryResponse response = mock(QueryResponse.class);
        ...
        this.mocker.getComponentUnderTest().search("MONDO:0001", 0, null, null);

        verify(this.server).query(argThat(new IsIdQuery()));
    }

    @Test
    public void testMondoDiseaseOntologySuggestTermsIsNotId() throws ComponentLookupException, SolrServerException,
        IOException
    {
        QueryResponse response = mock(QueryResponse.class);
        when(this.server.query(any(SolrParams.class))).thenReturn(response);
        ...
        this.mocker.getComponentUnderTest().search("MONDO:Test", 0, null, null);

        verify(this.server).query(argThat(new IsDisMaxQuery()));
    }

    @Test
    public void testMondoDiseaseOntologySuggestTermsMultipleWords() throws ComponentLookupException,
